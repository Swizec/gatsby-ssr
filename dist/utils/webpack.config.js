...
module.exports = async (program, directory, suppliedStage, webpackPort = 1500, pages = []) => {
  ...
  function entry() {
    switch (stage) {
      ...
      case `develop-ssr-html`:
        return {
          main: directoryPath(`.cache/develop-ssr-entry.js`)
        };
      case `build-css`:
        return {
    ...
    switch (stage) {
      ...
      case `develop-ssr-html`:
        return [new _webpack2.default.DefinePlugin({
          ...
          __PATH_PREFIX__: JSON.stringify(store.getState().config.pathPrefix),
          __POLYFILL__: store.getState().config.polyfill
        }), new _extractTextWebpackPlugin2.default(`build-ssr-html-styles.css`)];
      case `develop-html`:
  ...
  config.merge({
    ...
    target: stage === `build-html` || stage === `develop-html` || stage === `develop-ssr-html` ? `node` : `web`,
